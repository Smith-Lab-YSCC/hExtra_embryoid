---
title: "Pedroza, Gassologlu et al., 2023"
author: "Dias, N."
---
...
# Piechart
fig2a2 <- ggplot(data@meta.data, aes(Timepoint, fill = Cell.type)) + # Map x to developmental stage
...
```{r Fig 2C}
# Format data
f2c_data <- ref@meta.data %>% select(c("collapse", "orig.ident"))
f2c_data <- cbind(f2c_data, as.data.frame(ref@reductions$umap@cell.embeddings))

fig2c <- ggplot(f2c_data) + geom_point(aes(UMAP_1, UMAP_2, fill = collapse, shape = orig.ident, alpha = 0.7)) + scale_shape_manual(values=c(21, 22, 23)) + scale_fill_manual(values = ref_anno, breaks = names(ref_anno)) + theme_void() + NoLegend()
...
foreground <- as.data.frame(data@reductions$ref.umap@cell.embeddings)
foreground$Cell.type <- data$Cell.type
...
fig4_data <- subset(data, idents = c("PS-like", "Mesoderm-like", "PI-Epi"))
...
```{r EDFig 3A/B}
# A: UMAP by timepoint
edf3a <- DimPlot(data, group.by = "Cell.type", split.by = "Timepoint", reduction = "umap") + scale_color_manual(values = anno$colors, breaks = anno$order)
edf3a
...
edf3c <- DimPlot(data, reduction = "pca", group.by = "Cell.type", shuffle = TRUE) + scale_color_manual(values = anno$colors, breaks = anno$order)
edf3c

data$pc_1 <- as.data.frame(data@reductions$pca@cell.embeddings)[, "PC_1", drop = FALSE]
data$pc1 <- rank(data$pc_1)

edf3c2 <- ggplot(data@meta.data, aes(x = pc1,
                          y = Cell.type, 
                          colour = Cell.type)) +
...
# D: Reference heatmap
edfig3d_data <- as.matrix(table(ref$new.identity, ref$collapse))
...
celltypist_score$cell.ID <- rownames(celltypist_score)
celltypist_score$Cell.type <- md$Cell.type
...
data <- FindVariableFeatures(data, assay = "RNA")

ref_a <- AverageExpression(ref, features = VariableFeatures(data), assays = "RNA", group.by = "collapse")$RNA
dat_a <- AverageExpression(data, features = VariableFeatures(data), group.by = "Cell.type", assays = "RNA")$RNA
...
edfig3f_data <- edfig3f_data[as.character(unique(ref$collapse)), as.character(unique(data$Cell.type))]
...
# Box plots
edfig3g1 <- ggplot(data@meta.data, aes(x = Cell.type, y = DE_markers1, fill = Cell.type)) + 
...
edfig3g2 <- ggplot(data@meta.data, aes(x = Cell.type, y = PE_markers1, fill = Cell.type)) + 
...
# Bar plot
edfig3g3_data <- data@meta.data[data$Cell.type == "G1 Hypoblast" | data$Cell.type == "G2M/S Hypoblast" | data$Cell.type == "AVE-like", c("Endo_bi", "Cell.type")]
...
# Box plots
bmp_sub <- as.data.frame(t(as.data.frame(GetAssayData(object = data, assay = "RNA", slot = "data")[c("BMP4", "BMP2", "BMPR1A"),])))
bmp_sub$cell_ID <- rownames(bmp_sub)
bmp_sub <- join(bmp_sub, data@meta.data, by = "cell_ID")
...
mat_1 <- AverageExpression(subset(data, Timepoint == "D4" & Cell.type %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')), assays = "RNA", features = wnt, group.by = "Cell.type")$RNA 
mat_1 <- t(scale(t(mat_1[,anno$order[anno$order %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')]]))) 
mat_1[is.na(mat_1)] <- 0

mat_2 <- AverageExpression(subset(data, Timepoint == "D4" & Cell.type %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')), assays = "RNA", features = tgfnodal, group.by = "Cell.type")$RNA 
mat_2 <- t(scale(t(mat_2[,anno$order[anno$order %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')]])))
mat_2[is.na(mat_2)] <- 0

mat_3 <- AverageExpression(subset(data, Timepoint == "D4" & Cell.type %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')), assays = "RNA", features = mapk, group.by = "Cell.type")$RNA 
mat_3 <- t(scale(t(mat_3[,anno$order[anno$order %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')]])))
mat_3[is.na(mat_3)] <- 0

mat_4 <- AverageExpression(subset(data, Timepoint == "D4" & Cell.type %in% c("PI-Epi", 'PI-Epi.L', 'G1 Hypoblast', 'G2M/S Hypoblast')), assays = "RNA", features = fgf, group.by = "Cell.type")$RNA 
...
ave_hypo <- subset(data, idents = c("AVE-like", "G1 Hypoblast", "G2M/S Hypoblast"))
